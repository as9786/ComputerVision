...
from convert_eval_format import convert_data
...
def parse_args():
    ...
    parser.add_argument('--preds', type=str, default='/workspace/ojt/OCR/dataset/infer_final/preds.json', help='Input your prediction')
    parser.add_argument('--ans', type=str, default='/workspace/ojt/OCR/dataset/infer_final/answer.json', help = 'Input the answer')
...
def calc_deteval_metrics(pred_bboxes_dict, gt_bboxes_dict, transcriptions_dict=None,
    ...
    for sample_name in gt_bboxes_dict:
        ...
        f1_score = 2 * (perSampleMetrics[sample_name]['precision'] * perSampleMetrics[sample_name]['recall'] ) / (perSampleMetrics[sample_name]['recall'] +perSampleMetrics[sample_name]['precision'])
        print(sample_name)
        print(f"precision : {perSampleMetrics[sample_name]['precision']}")
        print(f"recall : {perSampleMetrics[sample_name]['recall']}")
        print(f"hmean : {perSampleMetrics[sample_name]['hmean']}")
...
def main(preds, ans):
    ...
    # Image name
    new_preds, new_ans = convert_data(preds, ans)

    # 평가
    result = calc_deteval_metrics(new_preds, new_ans)
    print(result['total'])
