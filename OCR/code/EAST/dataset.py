...
class SceneTextDataset(Dataset):
    def __init__(self, root_dir,
                 ...
                 augmentation=False,
                 normalize=True):
        with open(osp.join(root_dir, 'train.json'), 'r') as f:
            anno = json.load(f)
    ...
    def __getitem__(self, idx):
        ...
        if self.augmentation:
            funcs.append(random_choice_augmentations(0.5))
        image = np.array(add_pepper(image, 0.5))
        
        if self.binarization:
            _, image = cv2.threshold(image, cv2.THRESH_OTSU)  # binarization
        if self.normalize:
            funcs.append(A.Normalize(mean=(0.9, 0.89, 0.88), std=(0.15, 0.15, 0.15)))
